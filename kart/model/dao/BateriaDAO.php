...
class BateriaDAO extends AbstractDAO {
	...
	public function getQueryBase() {
		...
		" left outer join " . $this->getdbprexis () . $etapaDAO->tabelaAlias () . 
		" on " . $this->getalias () . ".idetapa =  " . $etapaDAO->idtabelaAlias () . 
		" left outer join " . $this->getdbprexis () . $pistaDAO->tabelaAlias () . 
		" on " . $this->getalias () . ".idpista =  " . $pistaDAO->idtabelaAlias () . 
		" left outer join " . $this->getdbprexis () . $campeonatoDAO->tabelaAlias () .
		" on " . $etapaDAO->getalias () . ".idcampeonato =  " . $campeonatoDAO->idtabelaAlias () . 
		" left outer join " . $this->getdbprexis () . $pontuacaoesquemaDAO->tabelaAlias () . 
		" on " . $this->getalias () . ".idpontuacaoesquema =  " . $pontuacaoesquemaDAO->idtabelaAlias ().
		" left outer join " . $this->getdbprexis () . $categoriaDAO->tabelaAlias () . 
		" on " . $this->getalias () . ".idcategoria =  " . $categoriaDAO->idtabelaAlias ();
		// echo $query;
		return $query;
	}
	...
	public function findAtivoBateriaByCampeonatoEtapa($campeonato, $idetapa) {
		...
		try {
			...
			$query .= " and " . $etapaDAO->idtabelaAlias () . " =   IFNULL( ? ," . $etapaDAO->idtabelaAlias () . " )  " . 
			" ORDER BY " . $etapaDAO->getordernome () . ", " . $this->getordernome ();
...
// 	   	     /   " on " . $this->getalias () . ".idpista =  " . $pistaDAO->idtabelaAlias () .
    	        ...
    	        " and ".$campeonatoDAO->idtabelaAlias () . " = ? ".
    	        " ORDER BY " . $etapaDAO->getordernome () . ", " . $this->getordernome ();
	        $this->con->setNumero ( 1, $campeonato );
	        $this->con->setsql ( $query );
	        //echo $this->con->getsql ( ) ;
