...
include_once PATHAPP . '/mvc/kart/model/bean/EventoBean.php';
class EventoDAO extends AbstractDAO {
	protected $dbprexis = "kart_";
	protected $alias = "evento";
	protected $tabela = "evento";
	protected $idtabela = "idevento";
	...
	protected $ordernome = "evento.nome";
	public function __construct($_conn) {
		parent::__construct ( $_conn );
	}
	public function update($bean) {
		$this->results = new EventoBean ();
		try {
			...
			$this->con->setNumero ( 20, $bean->gettipoevento () );
			$this->con->setNumero ( 21, $bean->getadicionarcentavos () );
			$this->con->setNumero ( 22, $bean->getid () );
			$this->con->setsql ( $query );
			Util::echobr ( 0, "EventoDAO update", $this->con->getsql () );
			$this->con->execute ();
	...
	public function findAll() {
		...
		try {
			...
			$this->con->setsql ( $query );
			Util::echobr ( 0, "EventoDAO findAll", $this->con->getsql () );
	...
	public function findAllAtivo() {
		...
		try {
			...
			" ORDER BY " . $this->ordernome;
			$this->con->setsql ( $query );
			Util::echobr ( 0, "EventoDAO findAllAtivo", $this->con->getsql () );
			$result = $this->con->execute ();
	...
	public function findBySigla($sigla) {
		...
		try {
			...
			$this->con->setsql ( $query );
			Util::echobr ( 0, 'EventoDAO findBySigla getsql', $this->con->getsql () );
			$result = $this->con->execute ();
	...
	public function findByTiposAtivos($tipos) {
		...
		if($qtdtipo>0){
			try {
				...
				$this->con->setsql ( $query );
				Util::echobr ( 0, 'EventoDAO findByTipos getsql', $this->con->getsql () );
				$result = $this->con->execute ();
	...
	public function atual() {
		...
		try {
			...
			$this->con->setsql ( $query );
			Util::echobr ( 0, "EventoDAO atual", $this->con->getsql () );
			$result = $this->con->execute ();
