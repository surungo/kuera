...
include_once PATHAPP . '/mvc/kart/model/dao/DescarteEtapaDAO.php';
...
class DescarteEtapaBusiness {
	...
	public function findAll() {
		...
		try {
			...
			$objDAO = new DescarteEtapaDAO ( $con );
	...
	public function findByDescarte($ranking) {
		$dbg = 0;
		$results = Array ();
		$con = null;
		$dsm = new DataSourceManager ();
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new DescarteEtapaDAO ( $con );
			Util::echobr ( $dbg, 'DescarteEtapaBusiness Util::getIdObjeto($ranking)', Util::getIdObjeto($descarte) );
			$results = $objDAO->findByDescarte ( $descarte );
		} catch ( Exception $ex ) {
			// rollback transaction
			$con->rollback ();
			$dsm->close ( $con );
	...
	public function deleteByIdDescarte($idDescarte) {
		...
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new DescarteEtapaDAO ( $con );
			Util::echobr ( 0, 'DescarteEtapaBusiness deleteByIdDescarte idDescarte', $idDescarte );
			$results = $objDAO->deleteByIdDescarte($idDescarte);
	...
	public function deletedescarteetapa($bean) {
		...
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new DescarteEtapaDAO ( $con );
			$results = $objDAO->deletedescarteetapa( $bean );
		} catch ( Exception $ex ) {
			// rollback transaction
			$con->rollback ();
			$dsm->close ( $con );
