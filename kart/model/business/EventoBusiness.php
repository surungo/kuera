...
include_once PATHAPP . '/mvc/kart/model/dao/EventoDAO.php';
...
class EventoBusiness {
	...
	public function findBySigla($sigla) {
		Util::echobr ( 0, 'EventoBusiness findBySigla $sigla', $sigla );
		$results = null;
		$con = null;
		$dsm = new DataSourceManager ();
		try {
	...
	public function atual() {
		...
		try {
			...
			// evento atual
			$eventoAtual = new EventoBean ();
			$results = $objDAO->atual ();
			$idAtual = Util::getIdObjeto ( $results );
			Util::echobr ( $dbg, 'EventoBusiness atual() $idAtual', $idAtual );
			
			if ($idAtual < 1) {
				// evento principal
				$parametroBean = new ParametroBean ();
				$parametroBusiness = new ParametroBusiness ();
				$eventoprincipal = $parametroBusiness->findByCodigo ( 'CAMPEONATO_PRINCIPAL' );
				
				Util::echobr ( $dbg, 'EventoBusiness atual() $eventoprincipal', $eventoprincipal );
				$results = $objDAO->findById ( $eventoprincipal );
			}
		} catch ( Exception $ex ) {
			// rollback transaction
			$con->rollback ();
