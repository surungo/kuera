...
include_once PATHAPP . '/mvc/kart/model/business/EtapaCampeonatoBusiness.php';
class RankingEtapaBusiness {
	...
	public function findAll() {
		...
		try {
			...
			$objDAO = new RankingEtapaDAO ( $con );
	...
	public function findByRanking($ranking) {
		...
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new RankingEtapaDAO ( $con );
			Util::echobr ( $dbg, 'RankingEtapaBusiness Util::getIdObjeto($ranking)', Util::getIdObjeto($ranking) );
			$results = $objDAO->findByRanking ( $ranking );
		} catch ( Exception $ex ) {
			// rollback transaction
			$con->rollback ();
			$dsm->close ( $con );
	...
	public function deleteByIdRanking($idRanking) {
		...
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new RankingEtapaDAO ( $con );
			Util::echobr ( 0, 'RankingEtapaBusiness deleteByIdRanking idRanking', $idRanking );
	...
	public function deleterankingetapa($bean) {
		...
		try {
			$con = $dsm->getConn (get_class($this));
			$objDAO = new RankingEtapaDAO ( $con );
			$results = $objDAO->deleterankingetapa( $bean );
		} catch ( Exception $ex ) {
			// rollback transaction
			$con->rollback ();
			$dsm->close ( $con );
