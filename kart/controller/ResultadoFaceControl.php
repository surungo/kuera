...
Util::echobr ( $dbg, 'PilotoBateriaControl count $seletapaCollection ', count($seletapaCollection) );
...
Util::echobr ( $dbg, 'PilotoBateriaControl count $selbateriaCollection ', count($selbateriaCollection ) );
...
$homenageado =  (isset ( $_POST ['homenageado'] )) ? $_POST ['homenageado'] : '';
if( $homenageado == '' && Util::getIdObjeto($seletapabean)>0){
 $homenageado = $seletapabean->getinfo();
}

Util::echobr ( $dbg, 'PilotoBateriaControl  $homenageado', $homenageado );
$bean->getpostlog ();
$editar = false;
$novo = false;
...
switch ($choice) {
	...
	case Choice::LISTAR :
		...
		$bean->setsort ( $clsort );
		Util::echobr ( $dbg, 'PilotoBateriaControl LISTAR  $bean', $bean );
		if(Util::getIdObjeto($selbateriabean)>0)
...
Util::echobr ( $dbg, 'PilotoBateriaControl $siteUrl', $siteUrl);
