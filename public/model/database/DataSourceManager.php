...
class DataSourceManager {
	...
	public function query($query) {
		...
		try {
			...
			mysql_set_charset ( PRJCHARSETMYSQL, $this->link );
			...
			mysql_query ( "SET NAMES '" . PRJCHARSETMYSQL . "'" );
			mysql_query ( 'SET character_set_connection=' . PRJCHARSETMYSQL );
			mysql_query ( 'SET character_set_client=' . PRJCHARSETMYSQL );
			mysql_query ( 'SET character_set_results=' . PRJCHARSETMYSQL );
		} catch ( Exception $e ) {
	...
	public function histTable() {
		...
		if( $operacao=="update" || $operacao=="delete" ){
			...
			//cria temporaria
			$result = mysql_query ( $createTempTable, $this->link );
			...
			if( $operacao=="update" ){
				...
				$result = mysql_query ( $updateTempTable, $this->link );
			}else{
				...
				//atualizar tabela temporaria
				$result = mysql_query ( $deleteTempTable, $this->link );
		...
		$result = mysql_query ( $querygravahistorico, $this->link );
	...
	private function deletarTabelasTemp(){
		...
		while ($nomes = mysql_fetch_array($tables)){
			...
			if($prefixArr=="temp"){
				...
				$result = mysql_query ( $querydrop, $this->link );
			}
